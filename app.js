...
async function loadDoctorsForHosp() {
    ...
    if (!sel) return;
    var users = await API.getUsers().catch(function() { return []; });
    var doctors = users.filter(function(u) { return u.active && (u.role === 'doctor' || (u.extra_roles && u.extra_roles.includes('doctor'))); });
    sel.innerHTML = '<option value="">Sélectionner un médecin</option>' +
...
async function searchPatientForHosp() {
    ...
    if (!patients.length) { div.innerHTML = '<div class="alert alert-danger">Patient non trouvé</div>'; div.style.display = 'block'; return; }
    var p = patients[0];
...
async function admitPatient() {
    ...
    try {
        ...
        // Notifications
        await notifyDepartment('nurse', '🛏️ Nouveau patient hospitalisé',
            patientName + ' — ' + (room||'chambre à définir') + '. Médecin: ' + (doctor||'non assigné'), '#1a6bca');
        await notifyAdmin('🛏️ Hospitalisation', patientName + ' admis. ' + (room?'Chambre: '+room:'') + ' Dépôt: ' + deposit + ' HTG');
    } catch(e) {}
}
...
async function loadHospitalizations(status) {
    ...
    try {
        ...
        container.innerHTML = hosps.map(function(h) {
            ...
            return '<div class="card mb-2" style="border-left-color:' + (stateColors[h.status]||'#6c757d') + ';">' +
                '<div class="d-flex justify-between align-center flex-wrap gap-10">' +
                '<div>' +
                '<h4 style="margin-bottom:4px;">' + h.full_name + '</h4>' +
                '<small class="text-muted">' +
                (h.room ? '<i class="fas fa-door-open"></i> ' + h.room + (h.bed ? ' / ' + h.bed : '') + ' | ' : '') +
                '<i class="fas fa-calendar"></i> Admis le ' + h.admission_date + ' (' + days + ' jour(s))' +
                (h.doctor ? ' | Dr. ' + h.doctor : '') +
                '</small><br>' +
                '<small class="text-muted"><i class="fas fa-notes-medical"></i> ' + (h.admission_reason||'-') + '</small>' +
                '</div>' +
                '<div style="text-align:right;">' +
                '<span class="badge" style="background:' + (stateColors[h.status]||'#6c757d') + ';color:#fff;">' + (stateLabels[h.status]||h.status) + '</span><br>' +
                '<div style="margin-top:6px;font-weight:700;color:' + (isDebt?'#dc3545':'#28a745') + ';font-size:.95rem;">' +
                (isDebt ? '⚠️ Dette: ' + Math.abs(balance).toLocaleString('fr-FR') : '✅ Solde: ' + balance.toLocaleString('fr-FR')) + ' HTG' +
                '</div></div></div>' +
                '<div class="mt-2">' +
                '<button class="btn btn-primary btn-sm" data-hid="' + h.id + '" data-hname="' + h.full_name + '" onclick="openHospDetail(this.dataset.hid,this.dataset.hname)">' +
                '<i class="fas fa-folder-open"></i> Dossier</button>' +
                (h.status === 'active' ? ' <button class="btn btn-danger btn-sm" data-hid="' + h.id + '" onclick="quickDischarge(this.dataset.hid)"><i class="fas fa-sign-out-alt"></i> Sortie</button>' : '') +
                '</div></div>';
        }).join('');
...
async function openHospDetail(hospId, patientName) {
    ...
    // Charger médicaments pour prescription
    ...
    if (rxSel) {
        rxSel.innerHTML = '<option value="">Sélectionner du stock...</option>' +
            meds.map(function(m) {
                return '<option value="' + m.id + '" data-price="' + m.price + '" data-stock="' + m.quantity + '">' +
                    m.name + ' — Stock: ' + m.quantity + ' — ' + m.price + ' HTG</option>';
            }).join('');
    }
...
async function refreshHospDetail() {
    ...
    var isDebt   = balance < 0;
    document.getElementById('hosp-financial-summary').innerHTML =
        '<div class="d-flex gap-15 flex-wrap">' +
        '<div style="background:#d4edda;padding:12px 18px;border-radius:10px;text-align:center;">' +
        '<strong style="font-size:1.1rem;color:#155724;">' + deposit.toLocaleString('fr-FR') + ' HTG</strong>' +
        '<br><small>Total déposé</small></div>' +
        ...
        '<div style="background:#d1ecf1;padding:12px 18px;border-radius:10px;text-align:center;">' +
        '<strong style="font-size:1.1rem;color:#0c5460;">$' + (Math.abs(balance)/state.exchangeRate).toFixed(2) + '</strong>' +
...
function showHospTab(tab) {
    ...
    document.querySelectorAll('.hosp-tab-btn').forEach(function(btn) {
        btn.style.color        = btn.dataset.tab === tab ? 'var(--primary)' : 'var(--muted)';
        btn.style.borderBottom = btn.dataset.tab === tab ? '3px solid var(--primary)' : '3px solid transparent';
    });
    document.getElementById('hosp-tab-' + tab).style.display = 'block';
...
async function saveNursingNote() {
    ...
    try {
        ...
        loadNursingNotes();
        // Notification médecin
        await notifyDepartment('doctor', '❤️ Nouveau suivi infirmier',
            _currentHospPatient.full_name + ' — État: ' + (data.generalState||'non précisé') +
            (data.temperature ? ' | T°: ' + data.temperature : '') +
            (data.bloodPressure ? ' | TA: ' + data.bloodPressure : ''), '#dc3545');
    } catch(e) {}
}
...
async function loadNursingNotes() {
    ...
    container.innerHTML = notes.map(function(n) {
        ...
        return '<div class="card mb-2" style="border-left-color:' + (stateColors[n.general_state]||'#17a2b8') + ';">' +
            '<div class="d-flex justify-between align-center">' +
            '<div><strong>' + new Date(n.created_at).toLocaleString('fr-FR') + '</strong>' +
            ' — Par: <em>' + (n.recorded_by||'-') + '</em></div>' +
            (n.general_state ? '<span class="badge" style="background:' + (stateColors[n.general_state]||'#17a2b8') + ';color:#fff;">' + n.general_state + '</span>' : '') +
            '</div>' +
            (vitals.length ? '<div class="mt-2" style="display:flex;gap:8px;flex-wrap:wrap;">' + vitals.map(function(v){return '<span class="badge badge-info">'+v+'</span>';}).join('') + '</div>' : '') +
            (n.notes ? '<p class="mt-2" style="font-size:.88rem;white-space:pre-wrap;">' + n.notes + '</p>' : '') +
            '</div>';
    }).join('');
}
...
function updateRxCost() {
    ...
    if (total > 0 && prev) {
        prev.innerHTML = '<div class="d-flex gap-10 mt-1">' +
            '<span class="badge badge-primary">Coût total: ' + total.toLocaleString('fr-FR') + ' HTG</span>' +
            '<span class="badge badge-info">$' + (total/state.exchangeRate).toFixed(2) + '</span>' +
            '</div>';
    }
}

document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('rx-qty')?.addEventListener('input', updateRxCost);
    document.getElementById('rx-price')?.addEventListener('input', updateRxCost);
});

async function savePrescription() {
    if (!_currentHospId || !_currentHospPatient) return;
    var medSel  = document.getElementById('rx-medication');
    var medId   = medSel.value;
    var medName = medId ? medSel.options[medSel.selectedIndex].text.split(' —')[0]
    ...
    try {
        ...
        toast('Prescription enregistrée — ' + (qty*price).toLocaleString('fr-FR') + ' HTG déduit du solde', 'success');
        ...
        loadPrescriptions();
        await refreshHospDetail();
        await notifyDepartment('nurse', '💊 Nouvelle prescription',
            _currentHospPatient.full_name + ' — ' + medName + ' ' + dosage + ' ' + freq +
            (note ? ' | Note: ' + note : ''), '#6f42c1');
    } catch(e) {}
}
...
async function loadPrescriptions() {
    ...
    container.innerHTML = rxs.map(function(rx) {
        ...
        return '<div class="card mb-2" style="border-left-color:#6f42c1;">' +
            '<div class="d-flex justify-between align-center flex-wrap gap-10">' +
            '<div>' +
            '<h5 style="margin-bottom:4px;color:#6f42c1;"><i class="fas fa-prescription"></i> ' + rx.medication_name + '</h5>' +
            '<div style="display:flex;gap:10px;flex-wrap:wrap;font-size:.85rem;">' +
            '<span><i class="fas fa-pills"></i> ' + rx.dosage + '</span>' +
            (rx.frequency ? '<span><i class="fas fa-clock"></i> ' + rx.frequency + '</span>' : '') +
            (rx.duration  ? '<span><i class="fas fa-calendar"></i> ' + rx.duration  + '</span>' : '') +
            '<span><i class="fas fa-route"></i> ' + (rx.route||'Orale') + '</span>' +
            '</div>' +
            (rx.note ? '<p style="font-size:.82rem;color:#856404;background:#fff3cd;padding:4px 8px;border-radius:4px;margin-top:6px;"><i class="fas fa-info-circle"></i> ' + rx.note + '</p>' : '') +
            '<small class="text-muted">Prescrit par ' + (rx.prescribed_by||'-') + ' — Qté: ' + rx.quantity + ' × ' + parseFloat(rx.price_per_unit||0).toLocaleString('fr-FR') + ' HTG = <strong>' + total.toLocaleString('fr-FR') + ' HTG</strong></small>' +
            '</div>' +
            '<div style="text-align:right;">' +
            '<span class="badge" style="background:' + (statusColors[rx.status]||'#ffc107') + ';color:' + (rx.status==='pending'?'#212529':'#fff') + ';">' + (statusLabels[rx.status]||rx.status) + '</span>' +
            (rx.status === 'pending' ?
...
async function markAdministered(rxId) {
    ...
    await apiCall(function() {
        return API.updateHospPrescription(rxId, { status: 'administered', administeredAt: now, administeredBy: state.currentUser.username });
    });
...
async function addDeposit() {
    ...
    try {
        ...
        loadDeposits();
        await refreshHospDetail();
        await notifyAdmin('💰 Dépôt hospitalisation', _currentHospPatient.full_name + ' — Dépôt: ' + amount.toLocaleString('fr-FR') + ' HTG');
    } catch(e) {}
}
...
async function loadDeposits() {
    ...
    container.innerHTML =
        '<div class="exchange-banner mb-2"><i class="fas fa-wallet"></i><span>Total déposé: <strong>' + total.toLocaleString('fr-FR') + ' HTG</strong></span></div>' +
...
async function loadHospServices() {
    ...
    container.innerHTML =
        '<div class="exchange-banner mb-2"><i class="fas fa-clipboard-list"></i><span>Total services: <strong>' + total.toLocaleString('fr-FR') + ' HTG</strong></span></div>' +
...
async function dischargePatient() {
    ...
    var msg = 'Confirmer la sortie de ' + _currentHospPatient.full_name + ' ?' + debtInfo;
    ...
    try {
        ...
        await notifyAdmin('🏥 Sortie hospitalisation', _currentHospPatient.full_name +
            (balance < 0 ? ' — Dette restante: ' + Math.abs(balance).toLocaleString('fr-FR') + ' HTG' : ' — Soldé'));
        closeHospDetail();
    } catch(e) {}
}
...
async function quickDischarge(hospId) {
    ...
    try {
        await apiCall(function() {
            return API.updateHospitalization(hospId, { status: 'discharged', dischargeDate: new Date().toISOString().split('T')[0] });
        });
