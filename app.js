...
function playNotifSound(type) {
    try {
        ...
        if (type === 'message') {
            // Son message: 2 bips rapides aigus
            ...
            // Son alerte: 3 bips forts descendants
            ...
            // Son notification: 1 bip doux + harmonie
            var freqs = [523, 659, 784]; // Do-Mi-Sol
...
// Demander permission son au premier clic utilisateur
...
function addLocalNotification(title, body, icon = 'fas fa-bell', color = '#1a6bca') {
    ...
    updateNotifBadge();
    // Son selon le type
    var soundType = icon && icon.includes('comment') ? 'message'
                  : color === '#dc3545' ? 'alert'
                  : 'notification';
...
// ─── TICKET PATIENT (Secrétariat + Médecin) ───────────────────
async function openPatientTicket(patientId, patientName) {
    var modal = document.getElementById('patient-ticket-modal');
    if (modal) modal.remove();
    modal = document.createElement('div');
...
async function loadTicketTransactions(patientId) {
    ...
    try {
        ...
        if (!txs.length) {
            body.innerHTML = '<div class="alert alert-info">Aucune transaction pour ce patient.</div>';
            return;
        }
        var totPaid   = txs.filter(function(t){return t.status==='paid';}).reduce(function(s,t){return s+parseFloat(t.amount);},0);
        var totUnpaid = txs.filter(function(t){return t.status==='unpaid';}).reduce(function(s,t){return s+parseFloat(t.amount);},0);
        body.innerHTML =
            '<div class="d-flex gap-10 mb-2 flex-wrap">' +
            '<span style="background:#d4edda;padding:6px 12px;border-radius:8px;font-size:.85rem;">✅ Payé: <strong>' + totPaid.toLocaleString('fr-FR') + ' HTG</strong></span>' +
            '<span style="background:#f8d7da;padding:6px 12px;border-radius:8px;font-size:.85rem;">⏳ Non payé: <strong>' + totUnpaid.toLocaleString('fr-FR') + ' HTG</strong></span>' +
            '</div>' +
            '<div class="table-container"><table><thead><tr><th>Date</th><th>Service</th><th>Montant</th><th>Statut</th><th>Action</th></tr></thead><tbody>' +
            txs.map(function(t) {
                var amt = parseFloat(t.amount);
                return '<tr>' +
                    '<td>' + (t.date||'-') + '</td>' +
                    ...
                    '<td><span class="' + (t.status==='paid'?'status-paid':'status-unpaid') + '">' + (t.status==='paid'?'Payé':'Non payé') + '</span></td>' +
                    '<td><button class="btn btn-xs btn-danger" data-tid="' + t.id + '" onclick="removeFromTicket(this.dataset.tid)">' +
                    '<i class="fas fa-trash"></i></button></td></tr>';
            }).join('') +
            '</tbody></table></div>';
    } catch(e) {
        body.innerHTML = '<div class="alert alert-danger">Erreur chargement</div>';
    }
}

async function loadTicketServices(type) {
    var sel = document.getElementById('ticket-service');
    var qtyDiv = document.getElementById('ticket-qty-div');
    if (!sel) return;
    var options = [];
    if (type === 'consultation') {
        options = state.consultationTypes.filter(function(c){return c.active;}).map(function(c){
            return { id: c.id, label: c.name, price: c.price };
        });
        if (qtyDiv) qtyDiv.style.display = 'none';
    } else if (type === 'lab') {
        options = state.labAnalysisTypes.filter(function(a){return a.active;}).map(function(a){
            return { id: a.id, label: a.name, price: a.price };
        });
        if (qtyDiv) qtyDiv.style.display = 'none';
    } else if (type === 'medication') {
        options = (state.medications||[]).map(function(m){
            return { id: m.id, label: m.name + ' (' + m.quantity + ' en stock)', price: m.price };
        });
        if (qtyDiv) qtyDiv.style.display = 'block';
    } else if (type === 'external') {
        options = state.externalServiceTypes.filter(function(s){return s.active;}).map(function(s){
            return { id: s.id, label: s.name, price: s.price };
        });
        if (qtyDiv) qtyDiv.style.display = 'none';
    }
    sel.innerHTML = options.map(function(o){
        return '<option value="' + o.id + '" data-price="' + o.price + '">' + o.label + ' — ' + parseFloat(o.price).toLocaleString('fr-FR') + ' HTG</option>';
    }).join('') || '<option>Aucun disponible</option>';
    onTicketServiceChange();
}

function onTicketTypeChange() {
    var type = document.getElementById('ticket-add-type').value;
    loadTicketServices(type);
}

function onTicketServiceChange() {
    var sel = document.getElementById('ticket-service');
...
async function addToTicket(patientId, patientName) {
    var type    = document.getElementById('ticket-add-type').value;
    var sel     = document.getElementById('ticket-service');
    ...
    var opt     = sel.options[sel.selectedIndex];
    if (!opt || !opt.value) { toast('Sélectionner un service', 'error'); return; }
    var serviceId = opt.value;
    var serviceName = opt.text.split(' —')[0];
    var serviceLabel = {consultation:'Consultation',lab:'Analyse',medication:'Médicament',external:'Service'}[type] || type;
    var totalPrice = type === 'medication' ? price * qty : price;
    var data = {
        patientId:   patientId,
        patientName: patientName,
        service:     serviceLabel + ': ' + serviceName,
        amount:      totalPrice,
        type:        type,
    };
    if (type === 'consultation') data.consultationTypeId = serviceId;
    if (type === 'lab')          data.analysisId         = serviceId;
    if (type === 'medication')   { data.medicationId = serviceId; data.quantity = qty; }
    try {
        await apiCall(function() { return API.addTransaction(data); });
        toast(serviceLabel + ' ajouté au ticket !', 'success');
        await loadTicketTransactions(patientId);
    } catch(e) {}
}

async function removeFromTicket(txId) {
    if (!confirm('Supprimer cette ligne du ticket ?')) return;
    try {
        await apiCall(function() { return API.deleteTransaction(txId); });
        toast('Ligne supprimée', 'warning');
        // Recharger le ticket
        var modal = document.getElementById('patient-ticket-modal');
        if (modal) {
            var pid = modal.querySelector('[data-pid]')?.dataset.pid;
            if (pid) await loadTicketTransactions(pid);
        }
    } catch(e) {}
}
    ...
    try {
        ...
        tbody.innerHTML = patients.map(function(p) {
            return '<tr>' +
                ...
                '<button class="btn btn-sm btn-primary" data-pid="' + p.id + '" data-pname="' + p.full_name + '" onclick="openPatientTicket(this.dataset.pid,this.dataset.pname)">' +
                '<i class="fas fa-receipt"></i> Ticket</button>' +
                '<button class="btn btn-sm btn-secondary" data-pid="' + p.id + '" onclick="printPatientCard(this.dataset.pid)">' +
...
function updateLabAnalysesSelect() {
    var container = document.getElementById('lab-analyses-selection');
    if (!container) return;
    var active = state.labAnalysisTypes.filter(function(a) { return a.active; });
    var PAGE = 10;
    var showing = PAGE;
    function renderLab() {
        var visible  = active.slice(0, showing);
        var remaining = active.length - showing;
        container.innerHTML = visible.map(function(a) {
            return '<label style="display:flex;align-items:center;gap:8px;margin-bottom:6px;cursor:pointer;">' +
                ...
                '<span class="result-type-badge ' + (a.result_type==='image'?'result-type-image':'result-type-text') + '">' +
                (a.result_type==='image'?'Image':'Texte') + '</span></label>';
        }).join('') +
        (remaining > 0 ?
            '<div style="text-align:center;margin-top:8px;">' +
            '<button class="btn btn-sm btn-secondary" id="lab-show-more">' +
            '<i class="fas fa-chevron-down"></i> Voir ' + remaining + ' de plus</button></div>'
        : (active.length > PAGE ?
            '<div style="text-align:center;margin-top:8px;">' +
            '<button class="btn btn-sm btn-secondary" id="lab-show-less">' +
            '<i class="fas fa-chevron-up"></i> Réduire</button></div>'
        : ''));
        var btnMore = document.getElementById('lab-show-more');
        var btnLess = document.getElementById('lab-show-less');
        if (btnMore) btnMore.addEventListener('click', function() { showing = active.length; renderLab(); });
        if (btnLess) btnLess.addEventListener('click', function() { showing = PAGE; renderLab(); });
    }
    renderLab();
}
...
function printDayInvoice(dayTxs, date, patientName, patientId) {
    var rate     = state.exchangeRate || 130;
    ...
    var rows = dayTxs.map(function(t) {
        ...
        return '<tr style="border-bottom:1px solid #eee;">' +
            ...
            '<td style="padding:6px 4px;text-align:center;">' + (t.status==='paid'?'✅ Payé':'⏳ Non payé') + '</td>' +
            '</tr>';
    }).join('');
    ...
    win.document.write(
        ...
        '<title>Facture ' + date + ' — ' + patientName + '</title>' +
        '<style>' +
        'body{font-family:Arial,sans-serif;max-width:380px;margin:0 auto;padding:16px;font-size:13px;}' +
        'h1{font-size:1.1rem;text-align:center;margin:0;}' +
        '.center{text-align:center;} .right{text-align:right;}' +
        'table{width:100%;border-collapse:collapse;}' +
        'th{background:#f0f4ff;padding:6px 4px;text-align:left;font-size:12px;}' +
        '.total-row{font-weight:bold;border-top:2px solid #333;background:#f8f9fa;}' +
        '.divider{border:none;border-top:1px dashed #999;margin:10px 0;}' +
        '@media print{button{display:none;}}' +
        '</style></head><body>' +
        '<div class="center">' +
        '<h1>' + hospName + '</h1>' +
        (hospAddr  ? '<p style="margin:2px 0;font-size:11px;">' + hospAddr  + '</p>' : '') +
        (hospPhone ? '<p style="margin:2px 0;font-size:11px;">Tél: ' + hospPhone + '</p>' : '') +
        '</div>' +
        '<hr class="divider">' +
        '<p style="margin:4px 0;"><strong>Patient:</strong> ' + patientName + ' <small>(#' + patientId + ')</small></p>' +
        '<p style="margin:4px 0;"><strong>Date:</strong> ' + date + '</p>' +
        '<p style="margin:4px 0;"><strong>Imprimé le:</strong> ' + new Date().toLocaleString('fr-FR') + '</p>' +
        '<hr class="divider">' +
        '<table><thead><tr>' +
        '<th>Service</th><th class="right">HTG</th><th class="right">USD</th><th class="center">Statut</th>' +
        '</tr></thead><tbody>' + rows + '</tbody>' +
        '<tfoot>' +
        '<tr class="total-row"><td colspan="2"><strong>Total journée</strong></td>' +
        '<td colspan="2" class="right"><strong>' + total.toLocaleString('fr-FR') + ' HTG — $' + (total/rate).toFixed(2) + '</strong></td></tr>' +
        (totalPaid > 0 ? '<tr><td colspan="2" style="color:#155724;">✅ Payé</td><td colspan="2" class="right" style="color:#155724;">' + totalPaid.toLocaleString('fr-FR') + ' HTG</td></tr>' : '') +
        (unpaid.length > 0 ? '<tr><td colspan="2" style="color:#721c24;">⏳ Non payé</td><td colspan="2" class="right" style="color:#721c24;">' + unpaid.reduce(function(s,t){return s+parseFloat(t.amount);},0).toLocaleString('fr-FR') + ' HTG</td></tr>' : '') +
        '</tfoot></table>' +
        '<hr class="divider">' +
        '<p class="center" style="font-size:11px;color:#6c757d;">Merci de votre confiance</p>' +
        '<div class="center" style="margin-top:16px;">' +
        '<button onclick="window.print()" style="background:#1a6bca;color:#fff;border:none;padding:10px 24px;border-radius:6px;font-size:14px;cursor:pointer;">' +
        '🖨️ Imprimer</button>' +
        '</div>' +
        '</body></html>'
    );
...
// ═══════════════════════════════════════════════════════════════
//  HOSPITALISATION — Toutes les fonctions
// ═══════════════════════════════════════════════════════════════
...
async function loadDoctorsForHosp() {
    ...
    if (!sel) return;
    var users = await API.getUsers().catch(function(){return[];});
    var docs  = users.filter(function(u){ return u.active && (u.role==='doctor'||(u.extra_roles||[]).includes('doctor')); });
    sel.innerHTML = '<option value="">Sélectionner un médecin</option>' +
        docs.map(function(d){ return '<option value="'+d.username+'">'+d.name+'</option>'; }).join('');
}
...
async function admitPatient() {
    var div = document.getElementById('hosp-patient-found');
    if (!div || !div.dataset.patientId) { toast('Chercher un patient d\'abord', 'error'); return; }
    ...
    try {
        await apiCall(function(){ return API.admitPatient({ patientId, room, bed, reason, doctorUsername: doctor, depositAmount: deposit }); });
        toast(patientName+' admis en hospitalisation !', 'success');
        toggleAdmitForm();
        ['hosp-patient-search','hosp-room','hosp-bed','hosp-reason','hosp-deposit'].forEach(function(id){
...
function renderNurseHospList(hosps) {
    ...
    container.innerHTML = hosps.map(function(h) {
        ...
        return '<div class="card mb-2" style="border-left:4px solid #28a745;padding:12px;">' +
            ...
            '<i class="fas fa-tasks"></i> Tâches à effectuer</button>' +
            '</div></div>';
    }).join('');
}
...
async function loadHospitalizations(status) {
    ...
    try {
        ...
        container.innerHTML = hosps.map(function(h) {
            ...
            var isDebt  = balance < 0;
            var days    = Math.max(0, Math.floor((new Date()-new Date(h.admission_date))/(1000*60*60*24)));
            return '<div class="card mb-2 hosp-card" style="border-left:4px solid '+(stateColors[h.status]||'#6c757d')+';padding:14px;">' +
                '<div class="d-flex justify-between align-center flex-wrap gap-10">' +
                '<div><h4 style="margin-bottom:4px;">'+h.full_name+'</h4>' +
                '<small class="text-muted">'+(h.room?h.room+(h.bed?' / '+h.bed:'')+' | ':'')+
                'Admis: '+h.admission_date+' ('+days+' j)'+(h.doctor?' | Dr. '+h.doctor:'')+'</small>' +
                '<br><small class="text-muted">'+(h.admission_reason||'-')+'</small></div>' +
                '<div style="text-align:right;">' +
                ...
                '<strong style="color:'+(isDebt?'#dc3545':'#28a745')+';font-size:.95rem;">'+(isDebt?'Dette: '+Math.abs(balance).toLocaleString('fr-FR'):'Solde: '+balance.toLocaleString('fr-FR'))+' HTG</strong>' +
                '</div></div>' +
                '<div class="d-flex gap-10 mt-2 flex-wrap">' +
...
async function openHospDetail(hospId, patientName) {
    ...
    var role2 = state.currentRole;
    var isDr   = role2==='doctor'||role2==='admin'||role2==='sub_admin';
    var isNrs  = role2==='nurse';

    // Formulaire d\'ordres — médecin/admin seulement
    var orderForm = document.getElementById('task-order-form');
    if (orderForm) orderForm.style.display = isDr ? 'block' : 'none';

    // Bouton admission — cacher pour infirmière
    var admitBtn2 = document.getElementById('btn-nouvelle-admission');
    if (admitBtn2) admitBtn2.style.display = isNrs ? 'none' : '';

    // Tabs prescription — cacher pour infirmière (l\'infirmière ne prescrit pas)
    document.querySelectorAll('.hosp-tab-btn').forEach(function(btn){
        if (btn.dataset.tab === 'prescriptions') {
            btn.style.display = isDr ? '' : 'none';
        }
    });

    // Ouvrir sur Tâches par défaut pour tout le monde
    showHospTab('tasks');
}
...
async function refreshHospDetail() {
    ...
    if (el) el.innerHTML =
        ...
        '<div style="background:#d4edda;padding:10px 16px;border-radius:10px;text-align:center;"><strong style="font-size:1.1rem;color:#155724;">'+deposit.toLocaleString('fr-FR')+' HTG</strong><br><small>Total déposé</small></div>' +
        '<div style="background:'+(isDebt?'#f8d7da':'#d4edda')+';padding:10px 16px;border-radius:10px;text-align:center;">' +
        '<strong style="font-size:1.1rem;color:'+(isDebt?'#721c24':'#155724')+'">'+(isDebt?'Dette: '+Math.abs(balance).toLocaleString('fr-FR'):'Solde: '+balance.toLocaleString('fr-FR'))+' HTG</strong>' +
        '<br><small>Solde actuel</small></div>' +
        '<div style="background:#d1ecf1;padding:10px 16px;border-radius:10px;text-align:center;"><strong style="color:#0c5460;">$'+(Math.abs(balance)/rate).toFixed(2)+'</strong><br><small>USD</small></div>' +
        (hosp.doctor?'<div style="background:#e2d9f3;padding:10px 16px;border-radius:10px;text-align:center;"><strong style="color:#6f42c1;">Dr. '+hosp.doctor+'</strong><br><small>Médecin</small></div>':'') +
        '</div>';
}

function closeHospDetail() {
    _currentHospId = null; _currentHospPatient = null;
...
async function saveNursingNote() {
    ...
    try {
        ...
        ['nur-temp','nur-bp','nur-pulse','nur-o2','nur-weight','nur-notes'].forEach(function(id){ var el=document.getElementById(id); if(el) el.value=''; });
        document.getElementById('nur-state').value = '';
        loadNursingNotes();
        notifyDepartment('doctor', 'Nouveau suivi infirmier',
            (_currentHospPatient.full_name||'Patient')+(data.generalState?' — État: '+data.generalState:'')+(data.temperature?' | T°: '+data.temperature:''), '#dc3545');
    } catch(e) {}
}
...
async function loadNursingNotes() {
    ...
    var stateColors = { Bon:'#28a745', Stable:'#17a2b8', Moyen:'#ffc107', Critique:'#dc3545', Ameliore:'#28a745' };
    container.innerHTML = notes.map(function(n) {
        var vitals = [n.temperature?'T: '+n.temperature+'C':null, n.blood_pressure?'TA: '+n.blood_pressure:null,
                      n.pulse?'Pouls: '+n.pulse:null, n.oxygen_sat?'O2: '+n.oxygen_sat+'%':null, n.weight?'Poids: '+n.weight+'kg':null].filter(Boolean);
        return '<div class="card mb-2" style="border-left:4px solid '+(stateColors[n.general_state]||'#17a2b8')+';">' +
            '<div class="d-flex justify-between align-center"><strong>'+new Date(n.created_at).toLocaleString('fr-FR')+'</strong>' +
            ' — <em>'+(n.recorded_by||'-')+'</em>' +
...
function updateRxCostPreview() {
    ...
    if (total > 0) {
        ...
        prev.innerHTML = '<span class="badge badge-primary">Cout: '+total.toLocaleString('fr-FR')+' HTG</span> ' +
            '<span class="badge '+(bal-total<0?'badge-danger':'badge-success')+'">Solde apres: '+(bal-total).toLocaleString('fr-FR')+' HTG</span>';
    } else {
        prev.innerHTML = '';
    }
}
...
async function savePrescription() {
    ...
    var medName = medId ? medSel.options[medSel.selectedIndex].text.split(' (')[0] : document.getElementById('rx-med-name').value.trim();
    ...
    if (!medName || !dosage) { toast('Medicament et dosage obligatoires', 'error'); return; }
    try {
        await apiCall(function(){ return API.addHospPrescription({ hospitalizationId:_currentHospId, patientId:_currentHospPatient.patient_id, medicationId:medId||null, medicationName:medName, dosage, frequency:freq, duration:dur, route, note, pricePerUnit:price, quantity:qty }); });
        toast('Prescription enregistree — '+(qty*price).toLocaleString('fr-FR')+' HTG deduit', 'success');
...
async function loadPrescriptions() {
    ...
    var stColors = { pending:'#ffc107', administered:'#28a745', skipped:'#dc3545' };
    var stLabels = { pending:'En attente', administered:'Administre', skipped:'Saute' };
    container.innerHTML = rxs.map(function(rx) {
        var total = parseFloat(rx.total_price||0);
        return '<div class="card mb-2" style="border-left:4px solid #6f42c1;padding:12px;">' +
            '<div class="d-flex justify-between align-center flex-wrap gap-10">' +
            '<div><h5 style="color:#6f42c1;margin-bottom:4px;">'+rx.medication_name+'</h5>' +
            '<div style="font-size:.82rem;display:flex;gap:8px;flex-wrap:wrap;">' +
            '<span>'+rx.dosage+'</span>'+(rx.frequency?'<span>'+rx.frequency+'</span>':'')+(rx.duration?'<span>'+rx.duration+'</span>':'')+'<span>'+(rx.route||'Orale')+'</span></div>' +
            (rx.note?'<p style="font-size:.82rem;background:#fff3cd;padding:4px 8px;border-radius:4px;margin-top:4px;">'+rx.note+'</p>':'') +
            '<small class="text-muted">Qte: '+rx.quantity+' x '+parseFloat(rx.price_per_unit||0).toLocaleString('fr-FR')+' = <strong>'+total.toLocaleString('fr-FR')+' HTG</strong> | '+(rx.prescribed_by||'-')+'</small></div>' +
            '<div style="text-align:right;">' +
            '<span class="badge" style="background:'+(stColors[rx.status]||'#ffc107')+';color:'+(rx.status==='pending'?'#212529':'#fff')+';padding:4px 8px;">'+(stLabels[rx.status]||rx.status)+'</span>' +
            (rx.status==='pending'?'<br><div class="d-flex gap-6 mt-1">' +
                '<button class="btn btn-xs btn-success" data-rxid="'+rx.id+'" onclick="markAdministered(this.dataset.rxid)"><i class="fas fa-check"></i> Administre</button>' +
                '<button class="btn btn-xs btn-secondary" data-rxid="'+rx.id+'" onclick="markSkipped(this.dataset.rxid)"><i class="fas fa-times"></i> Saute</button>' +
                '</div>':'') +
            '</div></div></div>';
    }).join('');
}
...
async function markAdministered(rxId) {
    await apiCall(function(){ return API.updateHospPrescription(rxId, { status:'administered', administeredAt:new Date().toISOString() }); });
    toast('Medicament administre !', 'success');
    loadPrescriptions();
}
async function markSkipped(rxId) {
    await apiCall(function(){ return API.updateHospPrescription(rxId, { status:'skipped' }); });
    toast('Dose sautee', 'warning');
    loadPrescriptions();
}
...
async function addDeposit() {
    ...
    if (isNaN(amount)||amount<=0) { toast('Montant invalide', 'error'); return; }
    try {
        await apiCall(function(){ return API.addHospDeposit({ hospitalizationId:_currentHospId, patientId:_currentHospPatient.patient_id, amount, note }); });
        toast('Depot de '+amount.toLocaleString('fr-FR')+' HTG enregistre !', 'success');
...
async function loadDeposits() {
    ...
    if (!container) return;
    if (!deps.length) { container.innerHTML = '<div class="alert alert-info">Aucun depot.</div>'; return; }
    var total = deps.reduce(function(s,d){return s+parseFloat(d.amount);},0);
    container.innerHTML =
        '<div style="background:#d4edda;padding:10px 16px;border-radius:8px;margin-bottom:10px;">Total depose: <strong>'+total.toLocaleString('fr-FR')+' HTG</strong></div>' +
        '<div class="table-container"><table><thead><tr><th>Date</th><th>Montant</th><th>Note</th><th>Par</th></tr></thead><tbody>' +
        deps.map(function(d){ return '<tr><td>'+new Date(d.created_at).toLocaleString('fr-FR')+'</td><td><strong style="color:#28a745;">+'+parseFloat(d.amount).toLocaleString('fr-FR')+' HTG</strong></td><td>'+(d.note||'-')+'</td><td>'+(d.created_by||'-')+'</td></tr>'; }).join('') +
        '</tbody></table></div>';
}
...
async function addHospService() {
    ...
    try {
        await apiCall(function(){ return API.addHospService({ hospitalizationId:_currentHospId, patientId:_currentHospPatient.patient_id, service, amount, note }); });
        toast('Service ajoute — '+amount.toLocaleString('fr-FR')+' HTG deduit', 'success');
...
async function loadHospServices() {
    ...
    if (!container) return;
    if (!svcs.length) { container.innerHTML = '<div class="alert alert-info">Aucun service ajoute.</div>'; return; }
    ...
    container.innerHTML =
        ...
        svcs.map(function(s){ return '<tr><td>'+new Date(s.created_at).toLocaleDateString('fr-FR')+'</td><td>'+s.service+'</td><td><strong style="color:#dc3545;">-'+parseFloat(s.amount).toLocaleString('fr-FR')+' HTG</strong></td><td>'+(s.note||'-')+'</td></tr>'; }).join('') +
        '</tbody></table></div>';
}
...
async function loadHospTasks() {
    ...
    if (!tasks.length) {
        var emptyMsg = isNurse
            ? '<div class="alert alert-success"><i class="fas fa-check-circle"></i> Aucune tâche en attente — tout est à jour !</div>'
            : '<div class="alert alert-info"><i class="fas fa-tasks"></i> Aucune tâche ordonnée. Utilisez le formulaire ci-dessus pour donner des indications à l\'infirmière.</div>';
...
async function dischargePatient() {
    ...
    var debtInfo = balance < 0 ? ' | Dette: '+Math.abs(balance).toLocaleString('fr-FR')+' HTG' : ' | Solde: '+balance.toLocaleString('fr-FR')+' HTG';
    if (!confirm('Confirmer la sortie de '+(_currentHospPatient.full_name||'le patient')+'?'+debtInfo)) return;
    try {
        await apiCall(function(){ return API.updateHospitalization(_currentHospId, { status:'discharged', dischargeDate:new Date().toISOString().split('T')[0], dischargeNote:note }); });
        toast((_currentHospPatient.full_name||'Patient')+' sorti(e) !', 'success');
        notifyAdmin('Sortie hospitalisation', (_currentHospPatient.full_name||'Patient')+(balance<0?' — Dette: '+Math.abs(balance).toLocaleString('fr-FR')+' HTG':' — Solde'));
        closeHospDetail();
    } catch(e) {}
}
...
async function quickDischarge(hospId) {
    ...
    try {
        await apiCall(function(){ return API.updateHospitalization(hospId, { status:'discharged', dischargeDate:new Date().toISOString().split('T')[0] }); });
...
// ─── PAGINATION LISTES ────────────────────────────────────────
function renderPagedList(items, containerId, renderFn, pageSize) {
    pageSize = pageSize || 10;
    window['_pg_' + containerId] = { items: items, fn: renderFn, size: pageSize, showing: pageSize };
    refreshPagedList(containerId);
}
function refreshPagedList(cid) {
    var st = window['_pg_' + cid];
    var el = document.getElementById(cid);
    if (!st || !el) return;
    var rows = st.items.slice(0, st.showing).map(st.fn).join('');
    var rem  = st.items.length - st.showing;
    if (rem > 0) {
        rows += '<tr><td colspan="10" style="text-align:center;padding:10px;">' +
            '<button class="btn btn-sm btn-secondary" data-cid="' + cid + '" onclick="showMorePaged(this.dataset.cid)">' +
            '<i class="fas fa-chevron-down"></i> Voir ' + rem + ' de plus (Total: ' + st.items.length + ')' +
            '</button></td></tr>';
    } else if (st.items.length > st.size) {
        rows += '<tr><td colspan="10" style="text-align:center;padding:8px;">' +
            '<button class="btn btn-sm btn-secondary" data-cid="' + cid + '" onclick="showLessPaged(this.dataset.cid)">' +
            '<i class="fas fa-chevron-up"></i> Réduire</button></td></tr>';
    }
    el.innerHTML = rows;
}
function showMorePaged(cid) {
    var st = window['_pg_' + cid];
    if (st) { st.showing = st.items.length; refreshPagedList(cid); }
}
function showLessPaged(cid) {
    var st = window['_pg_' + cid];
    if (st) { st.showing = st.size; refreshPagedList(cid); }
}

async function updateSettingsDisplay() {
    // Types de consultation
    document.getElementById('consultation-types-list').innerHTML =
        '<div class="table-container"><table><thead><tr><th>Nom</th><th>Prix (Gdes)</th><th>Description</th><th>Actif</th><th>Actions</th></tr></thead><tbody id="consultation-types-list-tb"></tbody></table></div>';
    renderPagedList(state.consultationTypes, 'consultation-types-list-tb', function(ct) {
        return '<tr>' +
    ...
    document.getElementById('vitals-types-list').innerHTML =
        '<div class="table-container"><table><thead><tr><th>Nom</th><th>Unité</th><th>Min</th><th>Max</th><th>Actif</th><th>Actions</th></tr></thead><tbody id="vitals-types-list-tb"></tbody></table></div>';
    renderPagedList(state.vitalTypes, 'vitals-types-list-tb', function(v) {
        return '<tr>' +
    ...
    document.getElementById('lab-analyses-types-list').innerHTML =
        '<div class="table-container"><table><thead><tr><th>Nom</th><th>Prix</th><th>Type</th><th>Actif</th><th>Actions</th></tr></thead><tbody id="lab-analyses-types-list-tb"></tbody></table></div>';
    renderPagedList(state.labAnalysisTypes, 'lab-analyses-types-list-tb', function(a) {
        return '<tr>' +
                '<td>' + a.name + '</td>' +
                '<td>' + a.price + ' HTG</td>' +
    ...
    document.getElementById('external-services-types-list').innerHTML =
        '<div class="table-container"><table><thead><tr><th>Nom</th><th>Prix (Gdes)</th><th>Actif</th><th>Actions</th></tr></thead><tbody id="es-tb"></tbody></table></div>';
    renderPagedList(state.externalServiceTypes, 'es-tb', function(s) {
        return '<tr>' +
                '<td>' + s.name + '</td>' +
                '<td>' + s.price + ' Gdes</td>' +
...
async function updateMedicationsSettingsList() {
    ...
    state.medications = meds;
    renderPagedList(meds, 'medications-settings-list', function(m) {
        return '<tr class="' + (m.quantity === 0 ? 'out-of-stock' : m.quantity <= m.alert_threshold ? 'low-stock' : '') + '">' +
            '<td><strong>' + m.name + '</strong>' + (m.generic_name && m.generic_name !== m.name ? '<br><small class="text-muted">' + m.generic_name + '</small>' : '') + '</td>' +
            '<td>' + (m.form||'-') + '</td>' +
            '<td>' + m.price + ' HTG</td>' +
