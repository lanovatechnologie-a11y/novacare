...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // Comparaison directe (pas de hash comme demandé)
...
app.post('/api/tickets', authenticate, async (req, res) => {
    try {
        ...
        if (!drawId || !drawName || !bets || bets.length === 0) {
            return res.status(400).json({ error: 'Données de ticket incomplètes' });
        }

        // Vérifier que l'utilisateur est un agent
        if (req.user.role !== 'agent') {
            return res.status(403).json({ error: 'Seuls les agents peuvent créer des tickets' });
        }

        // Vérifier si le tirage est actif
        ...
        // Mettre à jour les ventes du tirage
        await Draw.findOneAndUpdate(
            { drawId },
            { $inc: { sales: total } }
        );

        // Mettre à jour les limites de nombres
...
app.delete('/api/supervisor/tickets/:ticketId', authenticate, async (req, res) => {
    try {
        ...
        if (ticketAge > 10) {
            return res.status(400).json({ error: 'Seuls les tickets de moins de 10 minutes peuvent être supprimés' });
        }
...
app.post('/api/owner/users', authenticate, async (req, res) => {
    try {
        ...
        // Vérifier le superviseur si c'est un agent
        ...
        const user = await User.create({
            userId,
            name,
            password, // Pas de hash comme demandé
            role,
...
app.put('/api/owner/users/:userId/block', authenticate, async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            message: `Utilisateur ${blocked ? 'bloqué' : 'débloqué'}`,
            user 
        });
    } catch (error) {
...
app.put('/api/owner/draws/:drawId/status', authenticate, async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            message: `Tirage ${active ? 'activé' : 'désactivé'}`,
            draw 
        });
    } catch (error) {
        res.status(500).json({ error: 'Erreur serveur' });
...
app.post('/api/owner/draws/:drawId/results', authenticate, async (req, res) => {
    try {
        ...
        if (!lot1 || !lot2 || !lot3) {
            return res.status(400).json({ error: 'Les 3 résultats sont requis' });
        }
        ...
        for (const ticket of tickets) {
            ...
            for (const bet of ticket.bets) {
                // Logique de calcul des gains (à adapter selon vos règles)
        ...
        res.json({ 
            success: true, 
            message: 'Résultats publiés et gains calculés',
            draw 
        });
    } catch (error) {
    ...
    try {
        ...
        for (const number of numbers) {
            const blockedItem = await BlockedItem.create({
                ...
                reason: reason || 'Bloqué par le propriétaire',
        ...
        res.status(201).json({ 
            success: true, 
            message: `${numbers.length} nombre(s) bloqué(s)`,
            blockedItems 
        });
    } catch (error) {
        res.status(500).json({ error: 'Erreur serveur' });
